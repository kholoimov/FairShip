...
def _event_collection_size(tree, branch_name: str) -> int:
...
def _build_branch_summary(tree, branch_name: str) -> dict[str, float | int]:
    total = 0
    total_squared = 0
    nonempty_events = 0
    minimum = None
    ...
    mean = total / n_events if n_events else 0.0
    rms = math.sqrt(total_squared / n_events) if n_events else 0.0
    variance = max(0.0, (total_squared / n_events) - (mean * mean)) if n_events else 0.0

    return {
        "total": total,
        "nonempty_events": nonempty_events,
        "min_per_event": minimum if minimum is not None else 0,
        "max_per_event": maximum,
        "mean_per_event": mean,
        "rms_per_event": rms,
        "sigma_per_event": math.sqrt(variance),
    }


def summarize_simulation_file(input_file: Path) -> dict[str, object]:
...
def print_summary(summary: dict[str, object]) -> None:
    ...
    for metric_name, metric in metrics.items():
        ...
        print(f"  min_per_event: {metric['min_per_event']}")
        print(f"  max_per_event: {metric['max_per_event']}")
        print(f"  mean_per_event: {metric['mean_per_event']:.3f}")
        print(f"  rms_per_event: {metric['rms_per_event']:.3f}")
        print(f"  sigma_per_event: {metric['sigma_per_event']:.3f}")
...
def main() -> int:
    ...
    parser.add_argument("-o", "--output-json", help="Optional path to write the summary as JSON.")

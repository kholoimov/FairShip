...
parser.add_argument(
    "--noVertexing", dest="noVertexing", help="switch off vertexing", required=False, action="store_true"
)
parser.add_argument(
    "--noStrawSmearing",
    dest="withNoStrawSmearing",
    help="no smearing of distance to wire, default on",
    required=False,
    action="store_true",
)
parser.add_argument(
    "--withT0", dest="withT0", help="simulate arbitrary T0 and correct for it", required=False, action="store_true"
)
parser.add_argument(
    "-i",
    "--firstEvent",
...
h = {}
if withHists:
    ut.bookHist(h, "distu", "distance to wire", 100, 0.0, 5.0)
    ut.bookHist(h, "distv", "distance to wire", 100, 0.0, 5.0)
    ut.bookHist(h, "disty", "distance to wire", 100, 0.0, 5.0)
    ut.bookHist(h, "nmeas", "nr measurements", 100, 0.0, 50.0)
    ut.bookHist(h, "chi2", "Chi2/DOF", 100, 0.0, 20.0)

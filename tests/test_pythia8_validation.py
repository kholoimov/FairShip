...
DEFAULT_REFERENCE_JSON = Path("tests/reference/pythia8_reference_run.json")
...
def _run_workdir():
    workdir = os.environ.get("FAIRSHIP_PYTHIA8_TEST_WORKDIR")
    if workdir:
        return Path(workdir)
...
def _alienv_package_name(repo_root):
    package = os.environ.get("FAIRSHIP_PYTHIA8_ALIENV_PACKAGE", os.environ.get("FAIRSHIP_ALIENV_PACKAGE"))
    if package:
        return package
...
def _debug_enabled():
    return os.environ.get("FAIRSHIP_PYTHIA8_TEST_DEBUG", "").lower() in {"1", "true", "yes", "on"}
...
def _run_shell_command(command, workdir, timeout):
    ...
    print(f"\n[pythia8-test] running in {workdir}")
    print(f"[pythia8-test] command:\n{command}\n")
...
def _reference_summary_file():
    reference = os.environ.get("FAIRSHIP_PYTHIA8_REFERENCE_JSON")
    path = Path(reference) if reference else (_repo_root() / DEFAULT_REFERENCE_JSON)
    if not path.exists():
        pytest.fail(f"Pythia8 JSON reference does not exist: {path}")
    return path


def _simulation_command(tmp_path, tag):
    repo_root = _repo_root()
    n_events = os.environ.get("FAIRSHIP_PYTHIA8_TEST_EVENTS", "1000")
    debug = os.environ.get("FAIRSHIP_PYTHIA8_TEST_FAIRLOGGER_DEBUG", "0")
...
def test_run_pythia8_and_validate_output(tmp_path):
    workdir = _run_workdir()
    tag = os.environ.get("FAIRSHIP_PYTHIA8_TEST_TAG", "pytest_pythia8")
    ...
    (tmp_path / "pythia8.stdout").write_text(sim_result.stdout)
    (tmp_path / "pythia8.stderr").write_text(sim_result.stderr)
    (tmp_path / "pythia8.command").write_text(sim_command + "\n")

    assert sim_result.returncode == 0, (
        "Pythia8 simulation command failed\n"
    ...
    assert sim_file.exists(), f"Missing Pythia8 simulation output file: {sim_file}"
    assert geo_file.exists(), f"Missing Pythia8 geometry output file: {geo_file}"
    assert par_file.exists(), f"Missing Pythia8 parameter output file: {par_file}"

    validation_command = _validation_command(tmp_path, tag)
    validation_result = _run_shell_command(validation_command, workdir, 3600)

    (tmp_path / "pythia8_validation.stdout").write_text(validation_result.stdout)
    (tmp_path / "pythia8_validation.stderr").write_text(validation_result.stderr)
    (tmp_path / "pythia8_validation.command").write_text(validation_command + "\n")

    assert validation_result.returncode == 0, (
        "Pythia8 validation command failed\n"
    ...
    summary_file = tmp_path / f"sim_{tag}.validation.json"
    assert summary_file.exists(), f"Missing Pythia8 validation summary JSON: {summary_file}"

    summary = json.loads(summary_file.read_text())

    assert summary["input_file"] == str(sim_file)
    assert summary["n_events"] == int(os.environ.get("FAIRSHIP_PYTHIA8_TEST_EVENTS", "1000"))
    assert summary["metrics"]["mc_tracks"]["total"] > 0, "No MC tracks were recorded in Pythia8 output"

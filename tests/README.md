...
```bash
pytest tests/integration/test_simulation_references.py
```

To run cases in parallel with `pytest-xdist`:

```bash
pytest -n auto tests/integration/test_simulation_references.py
```

Or with the standard integration entrypoint:

```bash
pytest tests/integration/test_simulation_references.py -m integration
```
